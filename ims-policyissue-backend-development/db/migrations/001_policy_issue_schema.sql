...
-- E-001b: premium_rates (Sankalan Table)
-- Description: Premium rate table for PLI/RPLI products based on age, gender, term

CREATE TABLE premium_rates (
    rate_id BIGINT PRIMARY KEY DEFAULT nextval('policy_issue_seq'),
    ...
    CONSTRAINT chk_term_positive CHECK (term > 0),
    CONSTRAINT chk_rate_positive CHECK (rate_per_1000 > 0),
    UNIQUE(product_code, age, gender, term, effective_from)
);

COMMENT ON TABLE premium_rates IS 'E-001b: Premium rates (Sankalan) for BR-POL-001';
...
-- E-006b: aadhaar_sessions
-- Description: Temporary storage for Aadhaar authentication sessions
...
COMMENT ON TABLE aadhaar_sessions IS 'E-006b: Temporary Aadhaar session storage for authentication flow';
...
CREATE TABLE proposal_data_entry (
    ...
    -- Completion Flags
    insured_details_complete BOOLEAN DEFAULT FALSE,
    nominee_details_complete BOOLEAN DEFAULT FALSE,
    medical_details_complete BOOLEAN DEFAULT FALSE,
    declaration_complete BOOLEAN DEFAULT FALSE,
    proposer_details_complete BOOLEAN DEFAULT FALSE,
    policy_details_complete BOOLEAN DEFAULT FALSE,
    agent_details_complete BOOLEAN DEFAULT FALSE,
...
-- ============================================
-- E-007G: proposal_insured (Insured Details)
-- ============================================

CREATE TABLE proposal_insured (
    insured_id BIGINT PRIMARY KEY DEFAULT nextval('policy_issue_seq'),
    proposal_id BIGINT NOT NULL REFERENCES proposals(proposal_id) ON DELETE CASCADE,
    ...
    CONSTRAINT uq_proposal_insured UNIQUE (proposal_id)
);

COMMENT ON TABLE proposal_insured IS 'E-007G: Insured person detailed information';
