...
**Project Name:** Mobile App for Customers
...
The purpose of this document is to define the requirements for a mobile
application that enables PLI/RPLI customers to manage policies, make
payments, initiate claims, and access services conveniently on
smartphones.

## **2. Project Scope**

The mobile app will allow customers to:
...
## **3. Business Requirements**
  ...
  ID            Requirement       
  ------------- ----------------- ---------------------------------------
  FS_MAC_001    Customer          The portal shall allow new and existing
                Onboarding        customers to register using policy
                                  details and mobile OTP.

  FS_MAC_002    Customer          The system shall validate customer
                Onboarding        identity using Aadhaar or other
                                  government-approved KYC methods.

  FS_MAC_003    Secure Login &    The portal shall provide secure login
                Authentication    with username/password and OTP-based
                                  multi-factor authentication.

  FS_MAC_004    Secure Login &    The system shall support password reset
                Authentication    and account recovery mechanisms.

  FS_MAC_005    Customer          The portal shall display a personalized
                Dashboard         dashboard showing active policies,
                                  premium due dates, claim status, and
                                  notifications.

  FS_MAC_006    Customer          The dashboard shall provide quick
                Dashboard         access to key actions like payments and
                                  service requests.

  FS_MAC_007    Premium Payment   The portal shall allow customers to pay
                                  premiums online using multiple payment
                                  modes (UPI, Net Banking, Cards).

  FS_MAC_008    Premium Payment   The system shall generate and store
                                  digital receipts for each transaction.

  FS_MAC_009    Claim Initiation  The portal shall allow customers to
                                  initiate claims (maturity, death,
                                  surrender) by submitting required forms
                                  and documents.

  FS_MAC_010    Claim Initiation  The system shall validate claim
                                  eligibility and notify users of next
                                  steps.

  FS_MAC_011    Claim Tracking    The portal shall provide real-time
                                  tracking of claim status.

  FS_MAC_012    Claim Tracking    Customers shall receive automated
                                  updates via SMS/email at each stage of
                                  claim processing.

  FS_MAC_013    Notifications and The portal shall send timely
                Support           notifications for premium due dates,
                                  claim updates, and service request
                                  status.

  FS_MAC_014    Notifications and The system shall include a support
                Support           module with chatbot, FAQs, and
                                  ticket-based query resolution.

  FS_MAC_015    New Policy        The portal shall allow customers to
                Purchase          explore and initiate purchase of
                Initiation        PLI/RPLI policies.

  FS_MAC_016    New Policy        The system shall generate premium
                Purchase          quotes based on user inputs and policy
                Initiation        type.

  FS_MAC_017    Policy Purchase   The portal shall facilitate scheduling
                Finalization      of medical tests (if required) and
                                  submission of final documents.

  FS_MAC_018    Policy Purchase   The system shall issue policy documents
                Finalization      upon successful verification and
                                  approval.

  FS_MAC_019    Non-Financial     The portal shall allow submission of
                Service Requests  service requests that require physical
                -- Type A         verification at a post office.

  FS_MAC_020    Non-Financial     The system shall track and update the
                Service Requests  status of such requests.
                -- Type A         

  FS_MAC_021    Non-Financial     The portal shall allow submission of
                Service Requests  service requests authenticated via OTP
                -- Type B         (e.g., mobile/email update).

  FS_MAC_022    Non-Financial     The system shall process and confirm
                Service Requests  such requests instantly.
                -- Type B         

  FS_MAC_023    Policy            The portal shall allow customers to
                Commutation       request changes to premium amount,
                                  premium term, or sum assured.

  FS_MAC_024    Policy            The system shall validate requests
                Commutation       against underwriting rules and notify
                                  customers of approval or rejection.

  FS_MAC_025    Policy Conversion The portal shall allow eligible
                                  customers to convert their policy type.

  FS_MAC_026    Policy Conversion The system shall guide users through
                                  the conversion process and update
                                  policy records accordingly.

  FS_MAC_027    Session &         The portal shall implement secure
                Security          session management with automatic
                Management        logout on inactivity.

  FS_MAC_028    Session &         The system shall encrypt sensitive data
...
**Flow for the Mobile App for Customers:**
